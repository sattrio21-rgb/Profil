          ...
          <div className="max-w-2xl">
            ...
            <div className="mb-4">
              <h1 className="text-4xl sm:text-5xl font-bold text-white">
                {profile?.judul_bem || 'Menteri Seni dan Olahraga'}
              </h1>
              <p className="text-lg text-gray-400">
                {profile?.deskripsi_bem || 'Badan Eksekutif Mahasiswa Vokasi Universitas Brawijaya'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 sm:px-8">
          {pengalamans.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {pengalamans.map((item, index) => (
                <ScrollAnimate key={item.id} delay={index * 100}>
                  <div className="group bg-white/[0.03] border border-white/5 rounded-2xl overflow-hidden hover:border-blue-500/20 transition-all duration-300">
                    {/* Foto */}
                    {item.foto_url ? (
                      ...
                      <div className="aspect-video bg-blue-500/10 flex items-center justify-center">
                    ...
                    <div className="p-5">
                      ...
                      <p className="text-blue-400 font-medium text-sm">{item.jabatan}</p>
                    </div>
                  </div>
                </ScrollAnimate>
              ))}
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">Belum Ada Pengalaman BEM</h3>
              <p className="text-gray-500">Pengalaman BEM akan muncul di sini.</p>
            </div>
          )}
        </div>
      </section>
